...
func enviarEntradas(nr *NodoRaft) {
	for {
		if nr.Estado == lider {
			...
			for nodo := 0; nodo < len(nr.Nodos); nodo++ {
				if nodo != nr.Yo {
					...
					prevLogIndice := nr.nextIndice[nodo] - 1
					var prevLogMandato int
					var entradaAEnviar Entrada

					if prevLogIndice > 0 && prevLogIndice <= nr.getUltimoIndice() {
						prevLogMandato = nr.Log[prevLogIndice].Mandato
					} else if prevLogIndice > 0 && prevLogIndice > nr.getUltimoIndice() {
						prevLogIndice = 0
						prevLogMandato = 0
					}

					// Si hay entradas nuevas para enviar
					if nr.getUltimoIndice() >= nr.nextIndice[nodo] {
						entradaAEnviar = nr.Log[nr.nextIndice[nodo]]

					} else {
						// Envio de latido -> entrada vacía
						entradaAEnviar = Entrada{}
					}
		...
		// Esperamos un poco para el siguiente lote de entradas o latidos
		time.Sleep(tLatido)
	}
}
...
func soyCandidato(nr *NodoRaft) {
	...
	case <-nr.cambiarALider:
		...
		// Inicialización del líder
		nr.Mux.Lock()
		for i := 0; i < len(nr.Nodos); i++ {
			nr.nextIndice[i] = nr.getUltimoIndice() + 1
			nr.lastIndice[i] = 0
		}
		nr.Mux.Unlock()
...
func soyLider(nr *NodoRaft) {
	...
	case <-nr.cambiarASeguidor:
		...
		nr.Estado = seguidor
	case <-tempLatido.C:
		// El temporizador expira, envío el siguiente latido
	}
}
...
func (nr *NodoRaft) enviarOperacion(idNodo int, args *ArgAppendEntries,
	...
	if err != nil {
		...
		// 1. Si el seguidor tiene un mandato más alto, el líder debe dimitir
		if resultados.MandatoActual > nr.MandatoActual {
			actualizarMandato(&nr.MandatoActual, resultados.MandatoActual)
			nr.IdLider = IntNOINICIALIZADO
			// Dimisión inmediata (el select es no bloqueante si el canal está lleno)
			select {
			case nr.cambiarASeguidor <- true:
			default:
			}
			return true // El líder dimite
		}

		// Solo si el mandato es el mismo y sigo siendo el líder:
		if resultados.Exito {
			// 2. ÉXITO: El seguidor aceptó la(s) entrada(s). Actualizo sus índices.

			// El índice de la entrada que intentamos replicar con éxito
			var indiceEntradaReplicada int
			if args.Entradas != (Entrada{}) {
				// Replicó una entrada de log
				indiceEntradaReplicada = args.Entradas.Operacion.Indice
			} else {
				// Es un latido, no replicó una entrada específica, solo comprobó hasta el prevLogIndice
				indiceEntradaReplicada = args.prevLogIndice
			}

			// Si el índice de la entrada replicada es mayor que el lastIndice conocido, lo actualizamos.
			// Esto maneja el caso de latidos exitosos (indiceEntradaReplicada = prevLogIndice)
			if indiceEntradaReplicada > nr.lastIndice[idNodo] {
				nr.lastIndice[idNodo] = indiceEntradaReplicada
			}

			// nextIndice es la siguiente entrada a enviar, i.e., el replicado + 1
			nr.nextIndice[idNodo] = nr.lastIndice[idNodo] + 1

			// 3. Comprobar si se puede avanzar el Commit Index del Líder
			// La llamada a esta función se hace DENTRO del lock para asegurar el acceso al estado
			// Sin embargo, si la función auxiliar también usa lock, podría haber deadlock.
			// La implementaremos para que sea segura.
			// Por seguridad, llamamos a la función auxiliar que gestiona su propio Lock
			nr.comprobarYActualizarCommit()

		} else {
			// 4. FALLO: El seguidor tiene un log inconsistente. ¡Decremento nextIndice!
			// Decrementar nextIndice y reintentar con la entrada anterior.
			if nr.nextIndice[idNodo] > 1 { // nextIndice nunca debe ser menor que 1
				nr.nextIndice[idNodo]--
			} else {
				nr.nextIndice[idNodo] = 1 // Aseguramos que apunte al inicio del log.
			}
			nr.Logger.Printf("Mandato %d. Fallo en AE con %d. nextIndice para %d decrementado a %d\n",
				nr.MandatoActual, idNodo, idNodo, nr.nextIndice[idNodo])
		}
		return true
	}
}

// Nuevo: Comprueba si alguna entrada no comprometida ha sido replicada en la mayoría
// y, si es así, avanza el CommitIndice del líder.
func (nr *NodoRaft) comprobarYActualizarCommit() {
	// NOTA: Esta función DEBE ser llamada bajo Lock

	// Iteramos desde el CommitIndice actual + 1 hasta la última entrada del log
	for N := nr.CommitIndice + 1; N <= nr.getUltimoIndice(); N++ {
		// Regla: Solo podemos comprometer entradas de nuestro mandato actual
		// o si al menos una entrada de nuestro mandato actual ha sido comprometida.
		if nr.Log[N].Mandato == nr.MandatoActual {

			// Contar cuántos servidores (incluido el líder) tienen la entrada N
			replicasConEntradaN := 1 // El líder (yo) ya la tiene

			for idNodo := 0; idNodo < len(nr.Nodos); idNodo++ {
				if idNodo != nr.Yo {
					// lastIndice[idNodo] (matchIndex) es la entrada más alta replicada.
					if nr.lastIndice[idNodo] >= N {
						replicasConEntradaN++
					}
				}
			}

			// Si la mayoría tiene la entrada N, la comprometemos (CommitIndice = N)
			if replicasConEntradaN >= ((len(nr.Nodos) / 2) + 1) {
				nr.CommitIndice = N
				nr.Logger.Printf("Mandato %d. CommitIndice avanzado a %d (replicado en %d/%d nodos).\n",
					nr.MandatoActual, N, replicasConEntradaN, len(nr.Nodos))
			} else {
				// No se pudo comprometer N, no tiene sentido mirar N+1.
				break
			}
		} else {
			// Si es de un mandato anterior, se rompe la búsqueda, ya que Raft solo
			// permite avanzar el commit index a entradas con el mandato actual
...
func (nr *NodoRaft) someterOperacion(operacion TipoOperacion) (int, int,
	...
	if EsLider {
		...
		nr.Logger.Printf(
			"Mandato %d. Operación %d enviada para consenso.\n",
